...
import gov.nih.nlm.iti.figure.PanelLabelDetection.*;
...
public class PanelSplitter{
	...
	//	cvSaveImage(out_fname, inImg);
		...
		ArrayList<PanelLabelInfo_Final> candLabelSet = new ArrayList<PanelLabelInfo_Final>();
		
		PanelLabelInfo_Final finalLabelSet = new PanelLabelInfo_Final();
	...
	//	Arrays.fill(candLabelSet, 0);
...

		{
			...
			if (max_match_no > 0) {
				...
				finalLabelSet.labels = new ArrayList<PanelLabelInfo>();
				
				for (i = 0; i < candLabelSet.get(max_match_set_idx).noLabel; i++) {
					if (candLabelSet.get(max_match_set_idx).labels.get(i).label == 0)
						continue;
					
					PanelLabelInfo tmp_Info = new PanelLabelInfo();
	...
	*/	
	...
	//	Arrays.fill(candLabelSet, 0);
		...
		for (int i = 0; i < candLabelSet.size(); i++){
			...
			PanelLabelInfo_Final panelLabelInfo_Final = candLabelSet.get(i);
			ArrayList<PanelLabelInfo> labels = panelLabelInfo_Final.labels;
			for (int j = 0; j < labels.size(); j++ ){
				PanelLabelInfo label = labels.get(j);
				...
				if (!isDuplicate){
					...
					panel.top = label.top;
					panel.label = label.label; //JIE ZOU, added 4/28/2016
...
//		
//		String ocrModelFileName = "/Users/jie/Openi/Panel/programs-others/Daekeun/NN_OCR.model";
...
//		String imgName = "/Users/jie/Openi/Panel/programs-others/Daekeun/multipanel_images/1477-7819-6-23-1.jpg";
//
//		ArrayList<Final_Panel> finalSplitPanels = ps.panelSplitter(imgName, OCR_model);
//
//		System.out.println("Panel Splitter completes");	
//		
//		for (int i = 0; i < finalSplitPanels.size(); i++){
//			Final_Panel panel = finalSplitPanels.get(i);
